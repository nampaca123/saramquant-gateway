...
| --------------------------------------- | ------------------------------------------------------------ |
| RSI, 볼린저밴드 같은 용어를 모름        | 🟢/🟡/🔴 신호로 상태를 한눈에 보여줌                            |
...
Risk Badge는 **기술적 신호**와 **펀더멘털 건전성** 두 축을 종합하여 산출한다.

**🟢 Stable (안정)**

- RSI 40~60 (중립 구간)
- ATR이 20일 평균 이하 (변동성 낮음)
- 볼린저밴드 중심선 근처
- 부채비율이 업종 평균 이하, ROE 양수
- 사용자에게 보이는 문장: "현재 특별한 과열/과매도 신호 없음"

**🟡 Caution (주의)**

- RSI 30~40 또는 60~70 (과열/과매도 접근)
- 볼린저밴드 상단/하단 접촉
- ADX 25 이상 (강한 추세 진행 중)
- 또는: 부채비율이 업종 평균의 1.5배 이상이거나 ROE 음수
- 사용자에게 보이는 문장: "추세가 강하거나 과열에 접근 중. 주의 깊게 지켜볼 구간"

**🔴 High Risk (위험)**

- RSI 70 초과 또는 30 미만
- ATR 급등 (최근 20일 대비 1.5배 이상)
- 베타 1.5 이상 + ADX 25 이상
- 또는: 영업이익 적자 + 부채비율 200% 초과
- 사용자에게 보이는 문장: "변동성이 매우 크거나 과열/급락 구간. 신중한 판단 필요"

> ✦ 배지 산출 원칙: 기술적 신호와 펀더멘털 신호 중 더 나쁜 쪽을 따른다. 기술적으로 🟢이더라도 펀더멘털이 🔴이면 최종 배지는 🔴. 사용자에게는 "왜 이 배지인지"를 카드 형태로 분리해서 보여준다 (예: "가격 흐름은 안정적이지만 재무 건전성에 주의가 필요해요").
...
**섹션 1: 리스크 요약 카드 (가로 스크롤)**

기술적 지표와 펀더멘털 지표를 구분 없이 하나의 카드 흐름으로 제공한다. 사용자에게 "기술적"이니 "펀더멘털"이니 하는 분류는 보여주지 않는다. 그냥 "이 종목을 이해하기 위한 핵심 숫자 6~8개"를 자연스럽게 나열한다.

각 카드는 [지표명 / 현재 값 / 상태 아이콘 / 한 줄 해석] 구조

| 카드           | 예시                                                        |
| -------------- | ----------------------------------------------------------- |
| 시장 민감도    | 베타 1.32 · "시장이 1% 오르면 이 종목은 약 1.3% 움직여요"   |
| 위험 대비 성과 | 샤프 0.85 · "감수한 위험 대비 성과가 양호해요"              |
| 과열/과매도    | RSI 68 · 🟡 "과열 구간에 접근 중이에요"                      |
| 변동성         | ATR 1,250 · "최근 하루 평균 1,250원씩 움직여요"             |
| 추세 강도      | ADX 31 · "뚜렷한 추세가 진행 중이에요"                      |
| 주가 수준 ✦    | PER 12.3 · "1년 벌어들이는 이익 대비 12배 가격이에요"       |
| 자산 가치 ✦    | PBR 0.95 · "회사 장부 가치보다 약간 낮은 가격이에요"        |
| 수익 창출력 ✦  | ROE 14.2% · "자기 자본 대비 14% 수익을 내고 있어요"         |
| 재무 안정성 ✦  | 부채비율 85% · "빚이 자기 자본의 0.85배. 안정적인 편이에요" |

핵심 원칙: 숫자를 보여주되, 바로 옆에 일상어 해석을 붙인다.
...
[Next.js Frontend]
       ↓
[Spring Boot Gateway (Kotlin)]
  ├── 사용자 인증/관리
  ├── REST API 서빙 (모든 GET/POST 요청)
  ├── Anthropic API 호출 (LLM 해석)
  ├── Redis 캐시 (LLM 응답, 배지 캐시)
  └── PostgreSQL 직접 조회 (지표, 가격, 종목)
       ↓ (무거운 계산만 위임)
[Flask ML Server]
  ├── 몬테카를로 시뮬레이션
  ├── 포트폴리오 리스크 계산 (상관행렬, 분산 효과)
  ├── Risk Badge 산출 로직
  └── 기존 퀀트 파이프라인 (지표 계산, 데이터 수집)
```
...
| API                                | 용도                                                        | 화면      |
| ---------------------------------- | ----------------------------------------------------------- | --------- |
| GET /api/market-summary            | 시장별 지수 + 리스크 배지                                   | Home      |
| GET /api/stock-risk-movers         | 리스크 배지 변동 종목 리스트                                | Home      |
| GET /api/stock-screen              | 필터 조건 기반 종목 검색 (기술적 + 펀더멘털 필터 포함) ✦    | Screener  |
| GET /api/stocks/{symbol}           | 종목 기본 정보 + 기술적 지표 + 펀더멘털 지표 + 배지 ✦       | Detail    |
| GET /api/stocks/{symbol}/prices    | 가격 시계열 (차트용)                                        | Detail    |
| GET /api/stocks/{symbol}/benchmark | 벤치마크 대비 수익률                                        | Detail    |
| POST /api/ai/stock-analysis        | LLM 종목 해석 요청 (기술적 + 펀더멘털 데이터 주입)          | Detail    |
| POST /api/portfolio/analyze        | 포트폴리오 리스크 계산 (펀더멘털 건전성 요약 포함)          | Portfolio |
| POST /api/portfolio/simulate       | 몬테카를로 시뮬레이션                                       | Portfolio |
| POST /api/ai/portfolio-analysis    | LLM 포트폴리오 진단                                         | Portfolio |
| GET /api/indicators/meta           | 지표 설명 데이터 (필터 "?" 팝업용, 펀더멘털 지표 설명 포함) | Screener  |
...
변경의 핵심 철학은 **"펀더멘털을 별도 섹션으로 분리하지 않는다"**예요. 사용자 입장에서 "기술적 지표 탭 / 펀더멘털 탭"을 나누는 순간 복잡해지거든요. 그래서 Stock Detail의 리스크 카드 흐름 안에 PER, ROE, 부채비율을 자연스럽게 섞어 넣었고, AI 해석에서도 "가격 흐름은 이렇고, 회사 체력은 이렇다"를 한 호흡으로 설명하게 했어요.

Risk Badge에도 펀더멘털을 반영한 건, 아무리 차트가 안정적이어도 영업적자에 부채가 높은 회사를 🟢으로 보여주면 안 되기 때문이에요. "둘 중 더 나쁜 쪽을 따른다"는 원칙으로 잡았고, 대신 왜 그런 배지가 나왔는지는 카드로 나눠서 설명하게 해서 사용자가 혼란스럽지 않도록 했습니다.